...
def PointSpawn(points, radius = 0.25, spawn = True):
    ...
    for p in points:
        ...
        par_num += 1
        if not spawn:
            pos.append(ti.Vector3f(p[0],p[1],p[2]))
...
par_num = pos.shape[0]
pos = pos[0:par_num,:]
volume = np.repeat(4./3.*np.pi*0.125**3, par_num)
psize = np.repeat(0.125, par_num)
np.savetxt('SpherePacking.txt', np.column_stack((pos, volume, psize, psize, psize)), header="     PositionX            PositionY                PositionZ            Volume                PsizeX                PsizeY                PsizeZ", delimiter=" ")
