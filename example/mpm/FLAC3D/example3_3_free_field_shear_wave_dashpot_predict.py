...
OUTPUT_PATH = Path(
    os.environ.get(
        "EXAMPLE3_3_OUTPUT",
        "example/mpm/FLAC3D/results/example3_3_free_field_shear_wave_dashpot_predict",
    )
)
...
BOTTOM_DASHPOT_VELOCITY_MODE = os.environ.get("EXAMPLE3_3_BOTTOM_DASHPOT_VELOCITY_MODE", "predict").lower()
FF_COUPLING_VELOCITY_MODE = os.environ.get("EXAMPLE3_3_FF_COUPLING_VELOCITY_MODE", "current").lower()
MAX_PREVIOUS_FORCE_NODES = 20000
PREVIOUS_NODE_FORCE = None
CURRENT_DYNAMIC_DT = 0.0
...
@ti.func
def seismic_base_force_component(direction, area, velocity, nodal_force, nodal_mass, dt, shear_stress, rho, cp, cs):
    force = 0.0
    v_boundary = velocity[direction]
    if ti.static(BOTTOM_DASHPOT_VELOCITY_MODE == "predict"):
        if nodal_mass > 0.0:
            v_boundary += nodal_force[direction] / nodal_mass * dt
    if direction == SEISMIC_INPUT_DIRECTION:
        force += shear_stress * area
    if direction == 2:
        force += -rho * cp * v_boundary * area
    else:
        force += -rho * cs * v_boundary * area
    return force
...
def add_pair_impedance_and_free_field_force(
    ...
    """
    source_v = node[node_id, source_body].momentum[direction]
    target_v = node[node_id, target_body].momentum[direction]
    if ti.static(FF_COUPLING_VELOCITY_MODE == "predict"):
        if node[node_id, source_body].m > 0.0:
            source_v += node[node_id, source_body].force[direction] / node[node_id, source_body].m * CURRENT_DYNAMIC_DT
        if node[node_id, target_body].m > 0.0:
            target_v += node[node_id, target_body].force[direction] / node[node_id, target_body].m * CURRENT_DYNAMIC_DT
    dv = source_v - target_v
    coeff = rho * cs
    ...
    for nboundary in range(nconstraints):
        ...
        if iz == 0:
            ...
            if area > 0.0:
                ...
                nodal_force = node[node_id, body_id].force
                nodal_mass = node[node_id, body_id].m
                constraints[nboundary].traction = seismic_base_force_component(direction, area, vel, nodal_force, nodal_mass, CURRENT_DYNAMIC_DT, shear_stress, rho, cp, cs)
...
def dynamic_traction_constraints(sims, scene) -> None:
    global PREVIOUS_NODE_FORCE, CURRENT_DYNAMIC_DT
    CURRENT_DYNAMIC_DT = float(sims.delta)
...
mpm = MPM(title="FLAC3D Example 3.3 dashpot predict candidate")
    ...
    for body_id in BODY_IDS:
        ...
        for iy in range(gnum_y):
            for ix in range(gnum_x):
                ...
                theory = IMPEDANCE_VELOCITY_AMPLITUDE * STRESS_SCALE * wave
                rows.append({
                    "time": time_value,
                    ...
                    "bottom_dashpot_velocity_mode": BOTTOM_DASHPOT_VELOCITY_MODE,
                    "ff_coupling_velocity_mode": FF_COUPLING_VELOCITY_MODE,
                })
...
def write_wave_propagation_summary_candidate(path: Path, diagnostics: dict[str, object], histories: list[dict[str, float]]) -> None:
    if not histories:
        summary = {"status": "no_histories"}
    else:
        bottom_peak = max(abs(float(row.get("main_grid_base_vx", row.get("base_center_vx", 0.0)))) for row in histories)
        top_peak = max(abs(float(row.get("main_grid_top_vx", 0.0))) for row in histories)
        arrival = float(diagnostics.get("wave_arrival_time_at_top", 0.0) or 0.0)
        summary = {
            "bottom_actual_velocity_peak": bottom_peak,
            "top_actual_velocity_peak": top_peak,
            "theoretical_bottom_velocity_peak": IMPEDANCE_VELOCITY_AMPLITUDE * STRESS_SCALE,
            "source_corrected_arrival_error": arrival,
            "effective_Cs": DOMAIN_Z / max(arrival, 1.0e-30),
...
write_csv(OUTPUT_PATH / "bottom_input_diagnosis.csv", BOTTOM_ALL_NODE_SAMPLE_ROWS)
...
write_wave_propagation_summary_candidate(OUTPUT_PATH / "wave_propagation_summary.json", diagnostics, histories)
