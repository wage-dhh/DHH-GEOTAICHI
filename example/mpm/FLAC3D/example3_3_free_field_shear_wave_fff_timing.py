...
OUTPUT_PATH = Path(
    os.environ.get(
        "EXAMPLE3_3_OUTPUT",
        "example/mpm/FLAC3D/results/example3_3_error_decomposition/fff_timing/current_run",
    )
)
...
FFF_SOURCE_MODE = os.environ.get("EXAMPLE3_3_FFF_SOURCE_MODE", "current_total").lower()
...
def add_pair_impedance_and_free_field_force(
    ...
    if ti.static(FFF_SOURCE_MODE == "none"):
        free_field_force = 0.0
    elif ti.static(FFF_SOURCE_MODE == "previous_total"):
        free_field_force = previous_node_force[node_id, source_body][direction] * source_equivalent_force_scale
    elif ti.static(FFF_SOURCE_MODE == "half_total"):
        free_field_force = 0.5 * (previous_node_force[node_id, source_body][direction] + node[node_id, source_body].force[direction]) * source_equivalent_force_scale
    elif ti.static(FFF_SOURCE_MODE == "current_minus_bottom"):
        free_field_force = node[node_id, source_body].force[direction] * source_equivalent_force_scale
        # Side free-field boundary pairs are above the base in this Example 3.3 coupling path,
        # so bottom-force subtraction is zero here. The mode is kept explicit for diagnostics.
    elif ti.static(FFF_SOURCE_MODE == "current_internal_only"):
        # At this callback point GeoTaichi node.force is the assembled unbalanced nodal force
        # before this traction-constraint update is applied; for this elastic/no-gravity case
        # it is the closest available internal/free-field unbalanced-force proxy.
        free_field_force = node[node_id, source_body].force[direction] * source_equivalent_force_scale
    elif ti.static(FFF_SOURCE_MODE == "current_total"):
        free_field_force = node[node_id, source_body].force[direction] * source_equivalent_force_scale
    elif ti.static(FF_COUPLING_MODE == "impedance_plus_previous_ff"):
        free_field_force = previous_node_force[node_id, source_body][direction] * source_equivalent_force_scale
...
def write_diagnostics(path: Path, rows: list[dict[str, float]], particle_counts: dict[int, int], model=None) -> dict[str, object]:
    ...
    diagnostics: dict[str, object] = {
        ...
        "fff_source_mode": FFF_SOURCE_MODE,
...
FFF_PAIR_TIMESERIES_ROWS: list[dict[str, float | int | str]] = []
FFF_COMPONENT_TIMESERIES_ROWS: list[dict[str, float | int | str]] = []


def collect_fff_timing_rows(model, pair_rows, component_rows) -> None:
    nodal_velocity = model.scene.node.momentum.to_numpy()
    nodal_force = model.scene.node.force.to_numpy()
    nodal_mass = model.scene.node.m.to_numpy()
    ...
    # Main/free-field side pairs only. This is a diagnostic reconstruction of the same
    # public manual-style terms, not an internal FLAC3D object dump.
    pair_id = 0
    for iz in range(1, TOP_Z_NODE + 1):
        for ix, side_body, normal_dir, plane in ((LEFT_X_NODE, X_SIDE_FF_BODY_ID, 0, 'x_left'), (RIGHT_X_NODE, X_SIDE_FF_BODY_ID, 0, 'x_right')):
            for iy in range(FRONT_Y_NODE, BACK_Y_NODE + 1):
                nid = ix + iy * gnum_x + iz * gnum_x * gnum_y
                area = node_area_2d_py(iy, iz, FRONT_Y_NODE, BACK_Y_NODE, 0, TOP_Z_NODE)
                vm = float(nodal_velocity[nid, MAIN_BODY_ID, 0]); vf = float(nodal_velocity[nid, side_body, 0])
                coeff = DENSITY * (P_WAVE_VELOCITY if normal_dir == 0 else SHEAR_WAVE_VELOCITY)
                dash = coeff * area * (vf - vm)
                fff = float(nodal_force[nid, side_body, 0])
                total = dash + (0.0 if FFF_SOURCE_MODE == 'none' else fff)
                opp = -dash
                pair_rows.append({'time':time_value,'pair_id':pair_id,'plane':plane,'main_node_id':nid,'ff_node_id':nid,'v_main_x':vm,'v_ff_x':vf,'relative_velocity_x':vm-vf,'dashpot_force_x':dash,'Fff_x':fff,'total_force_to_main_x':total,'opposite_force_to_ff_x':opp,'action_reaction_residual_x':total+opp,'energy_flux_main':total*vm,'energy_flux_ff':opp*vf,'energy_flux_residual':total*vm+opp*vf})
                pair_id += 1
        for iy, side_body, normal_dir, plane in ((FRONT_Y_NODE, Y_SIDE_FF_BODY_ID, 1, 'y_front'), (BACK_Y_NODE, Y_SIDE_FF_BODY_ID, 1, 'y_back')):
            for ix in range(LEFT_X_NODE, RIGHT_X_NODE + 1):
                if not in_main_xz_profile_node_py(ix, iz):
                    continue
                nid = ix + iy * gnum_x + iz * gnum_x * gnum_y
                area = main_xz_profile_nodal_area_py(ix, iz)
                vm = float(nodal_velocity[nid, MAIN_BODY_ID, 0]); vf = float(nodal_velocity[nid, side_body, 0])
                ...
                pair_rows.append({'time':time_value,'pair_id':pair_id,'plane':plane,'main_node_id':nid,'ff_node_id':nid,'v_main_x':vm,'v_ff_x':vf,'relative_velocity_x':vm-vf,'dashpot_force_x':dash,'Fff_x':fff,'total_force_to_main_x':total,'opposite_force_to_ff_x':opp,'action_reaction_residual_x':total+opp,'energy_flux_main':total*vm,'energy_flux_ff':opp*vf,'energy_flux_residual':total*vm+opp*vf})
                pair_id += 1
    for body_id, body_level in ((X_SIDE_FF_BODY_ID,'x_side_ff'), (Y_SIDE_FF_BODY_ID,'y_side_ff'), (CORNER_FF_BODY_ID,'corner_ff')):
        for iz in range(0, TOP_Z_NODE + 1):
            for iy in range(gnum_y):
                for ix in range(gnum_x):
                    nid = ix + iy * gnum_x + iz * gnum_x * gnum_y
                    ftotal = float(nodal_force[nid, body_id, 0])
                    if abs(ftotal) < 1e-16 and iz != 0:
                        continue
                    area = bottom_nodal_area_py(body_id, ix, iy, gnum_x, gnum_y) if iz == 0 else 0.0
                    wave = wave_factor(time_value)
                    bottom_input = area * DSTRESS_AMPLITUDE * STRESS_SCALE * wave if area > 0 else 0.0
                    vx = float(nodal_velocity[nid, body_id, 0])
                    bottom_dash = -DENSITY * SHEAR_WAVE_VELOCITY * vx * area if area > 0 else 0.0
                    # Available decomposition: total nodal force minus explicitly reconstructable base terms.
                    internal_proxy = ftotal - bottom_input - bottom_dash
                    recon = internal_proxy + bottom_input + bottom_dash
                    component_rows.append({'time':time_value,'body_level':body_level,'node_id':nid,'coordinate':f'({ix},{iy},{iz})','is_boundary_pair_node':iz>0,'is_bottom_node':iz==0 and area>0,'internal_force_x':internal_proxy,'body_force_x':0.0,'bottom_input_force_x':bottom_input,'bottom_dashpot_force_x':bottom_dash,'side_coupling_force_x':0.0,'corner_coupling_force_x':0.0,'total_node_force_x':ftotal,'reconstructed_total_force_x':recon,'force_reconstruction_error':ftotal-recon})


def per_step_update() -> None:
    wrote_history = collect_histories(mpm, histories, previous_history)
    if wrote_history:
        collect_fff_timing_rows(mpm, FFF_PAIR_TIMESERIES_ROWS, FFF_COMPONENT_TIMESERIES_ROWS)
...
write_csv(OUTPUT_PATH / "fff_pair_force_balance_timeseries.csv", FFF_PAIR_TIMESERIES_ROWS)
write_csv(OUTPUT_PATH / "fff_force_component_timeseries.csv", FFF_COMPONENT_TIMESERIES_ROWS)
if DEBUG_BOUNDARY:
