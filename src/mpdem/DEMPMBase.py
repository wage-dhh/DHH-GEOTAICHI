...
class Solver:
    ...
    def save_file(self, mscene:MPMScene, dscene: DEMScene):
        ...
        print(
            f'# {current_time} # Step = {self.sims.current_step}    Save Number = {self.sims.current_print}    Simulation time = {self.sims.current_time}\n')
    ...
    def CouplingSolver(self, mscene: MPMScene, dscene: DEMScene):
        ...
        while self.sims.current_time <= self.sims.time:
            self.engine.dscene.wall.contact_force.fill(0)
            self.engine.dscene.wall.contact_force_dem.fill(0)
            self.engine.dscene.wall.contact_force_mpm.fill(0)
