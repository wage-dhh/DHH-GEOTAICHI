...
class BodyReader(object):
    ...
    def add_txt_body(self, scene: myScene, template):
        ...
        if self.sims.dimension == 3:
            # 0-based 第 7/8/9 列
            init_v = np.ascontiguousarray(
                particle_cloud[0:particle_number, [7, 8, 9]]
            ).astype(np.float32)  # shape = (particle_number, 3)
        else:
            # 二维例子：取第 7/8 列
            init_v = np.ascontiguousarray(
                particle_cloud[0:particle_number, [6, 7]]
            ).astype(np.float32)
        
        if self.sims.dimension == 3:
            fix_v = vec3u8([
                DictIO.GetEssential(self.FIX, is_fix)
                for is_fix in fix_v_str
            ])
        else:
            fix_v = vec2u8([
                DictIO.GetEssential(self.FIX, is_fix)
    ...
    def set_polygons(self, contact: DEMContact, body_dict):
        ...
        elif isinstance(vertices, (list, tuple, np.ndarray)):
