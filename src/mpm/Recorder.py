...
class WriteFile:
    ...
    def VisualizeParticle(self, sims: Simulation, position, velocity):
    ...
    def MonitorParticleCoupling(self, sims: Simulation, scene: myScene):
        ...
        #output.update({'stress': stress, 'radius': radius, 'external_force': external_force, 'velocity_gradient': velocity_gradient, 'state_vars': state_vars})###mingliu
       # output.update({'state_vars': state_vars})  ###mingliu
        ...
        self.visualizeParticle(sims, output['position'], output['velocity'])
    ...
    def MonitorIncompressibleParticleCoupling(self, sims: Simulation, scene: myScene):
        ...
        #self.visualizeParticle(sims, output['position'], output['velocity'], output['volume'], state_vars)########mingliu
        self.visualizeParticle(sims, output['position'], output['velocity'])
        np.savez(self.particle_path+f'/MPMParticle{sims.current_print:06d}', **output)
    ...
    def MonitorParticle(self, sims: Simulation, scene: myScene):
        ...
        velocity_gradient = scene.particle.velocity_gradient.to_numpy()[0:scene.particleNum[0]] 
        state_vars = scene.material.get_state_vars_dict(0, scene.particleNum[0])
        #output.update({'stress': stress, 'velocity_gradient': velocity_gradient, 'state_vars': state_vars})######mingliu
    ...
    def MonitorParticleBase(self, sims: Simulation, scene: myScene, particle_num):
        ...
        '''
        return {'t_current': sims.current_time, 'position': position, 'velocity': velocity}
