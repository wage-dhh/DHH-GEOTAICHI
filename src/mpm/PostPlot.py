...
def write_vtk_file(sims: Simulation, start_file, end_file, read_path, write_path, kwargs):
    ...
    for printNum in range(start_file, end_file):
        ...
        if DictIO.GetAlternative(kwargs, "write_bodyID", False):#########mingliu
            bodyID = np.ascontiguousarray(DictIO.GetEssential(particle_info, "bodyID"))
            data.update({"bodyID": bodyID})
        if DictIO.GetAlternative(kwargs, "write_volume", False):####mingliu
        ...
        if DictIO.GetAlternative(kwargs, "write_mean_stress", False):####mingliu
        ...
        if DictIO.GetAlternative(kwargs, "write_strain_component", False):####mingliu
        ...
        if DictIO.GetAlternative(kwargs, "write_stress_component", False):####mingliu
            stress = DictIO.GetEssential(particle_info, "stress")
        ...
        if DictIO.GetAlternative(kwargs, "write_state_variables", False):####mingliu
            state_vars = np.ascontiguousarray(DictIO.GetEssential(particle_info, "state_vars"))
            data.update(state_vars.item())
        if "normal" in particle_info and DictIO.GetAlternative(kwargs, "write_outer_norm", False):####mingliu
        ...
        if "external_force" in particle_info and DictIO.GetAlternative(kwargs, "write_external_force", False):####mingliu
