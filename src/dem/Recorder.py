...
class WriteFile:
    ...
    def MonitorParticle(self, sims: Simulation, scene: myScene):
        ...
        '''
        output = {'t_current': sims.current_time,
    ...
    def MonitorClump(self, sims: Simulation, scene: myScene):        
        ...
        pebble_num = scene.particleNum[0]
        ...
        x = np.ascontiguousarray(scene.particle.x.to_numpy()[0: pebble_num])
        rad = np.ascontiguousarray(scene.particle.rad.to_numpy()[0: pebble_num])
        multisphereIndex = np.ascontiguousarray(scene.particle.multisphereIndex.to_numpy()[0: pebble_num])
        np.savez(self.particle_path+f'/DEMClump{sims.current_print:06d}', t_current=sims.current_time, body_num = clump_num, 
                                                                                ...
                                                                                acceleration=acceleration, angular_moment=angular_moment, velocity=velocity, omega=omega, quanternion=quanternion,
    ...
    def MonitorPatch(self, sims: Simulation, scene: myScene):
        ...
        norm = np.ascontiguousarray(scene.wall.norm.to_numpy()[0: scene.wallNum[0]])
        contact_force = np.ascontiguousarray(scene.wall.contact_force.to_numpy()[0: scene.wallNum[0]])
        contact_force_mpm = np.ascontiguousarray(scene.wall.contact_force_mpm.to_numpy()[0: scene.wallNum[0]])
        contact_force_dem = np.ascontiguousarray(scene.wall.contact_force_dem.to_numpy()[0: scene.wallNum[0]])
        #print(contact_force)
        self.visualizeWall(sims, point1, point2, point3)
        np.savez(self.wall_path+f'/DEMWall{sims.current_print:06d}', t_current=sims.current_time, body_num=scene.wallNum[0], active=active,
                                                                     wallID=wallID, materialID=materialID, point1=point1, point2=point2, point3=point3,
                 norm=norm,contact_force_dem=contact_force_dem,contact_force_mpm=contact_force_mpm,contact_force=contact_force)
