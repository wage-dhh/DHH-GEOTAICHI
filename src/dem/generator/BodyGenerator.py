...
class ParticleGenerator(object):
    ...
    def generate_template_multispheres(self, scene: myScene, template): 
        ...
        max_radius = DictIO.GetEssential(template, "MaxRadius", "Radius")
        orientation = DictIO.GetAlternative(template, "BodyOrientation", None)
        clump_file_path = DictIO.GetAlternative(template, "ClumpPacking", None)
        pebble_file_path = DictIO.GetAlternative(template, "PebblePacking", None)
        set_orientations = set_orientation(orientation)  
        ...
        if self.write_file:
            self.write_clump_text(template_ptr, start_body_num, end_body_num, particleNum, clump_file_path, pebble_file_path)
        else:
    ...
    def write_clump_text(self, template_ptr: ClumpTemplate, start_body_num, end_body_num, particleNum,clump_file_path,pebble_file_path):
        ...
        '''
        if not os.path.exists(clump_file_path):
            np.savetxt(clump_file_path, np.column_stack((position, radius, orientation, inertia_vol, pebbleIndex1, pebbleIndex2)), header="     PositionX            PositionY                PositionZ                Radius                OrientX                OrientY                OrientZ                moi_volX                moi_volY                moi_volZ                startIndex                endIndex", delimiter=" ")
        else:
            with open(clump_file_path, 'w') as file:
                np.savetxt(file, np.column_stack((position, radius, orientation, inertia_vol, pebbleIndex1, pebbleIndex2)), delimiter=" ")

        if not os.path.exists(pebble_file_path):
            np.savetxt(pebble_file_path, np.column_stack((particle_pos, particle_rad, multisphereIndex)), header="     PositionX            PositionY                PositionZ                Radius                multisphereIndex", delimiter=" ")
        else:
            with open(pebble_file_path, 'w') as file:
                np.savetxt(file, np.column_stack((particle_pos, particle_rad, multisphereIndex)), delimiter=" ")
        '''
        clump_data = np.column_stack((position, radius,
                                      orientation, inertia_vol,
                                      pebbleIndex1, pebbleIndex2))
        pebble_data = np.column_stack((particle_pos, particle_rad,
                                       multisphereIndex))

        # -------------------------------------------------
        # 2. 确保目录存在
        # -------------------------------------------------
        for path in (clump_file_path, pebble_file_path):
            Path(path).parent.mkdir(parents=True, exist_ok=True)

        # -------------------------------------------------
        # 3. 写 clump 文件（首写带表头，其余覆盖或追加均可）
        # -------------------------------------------------
        first_clump = not os.path.isfile(clump_file_path)
        np.savetxt(
            clump_file_path, clump_data,
            header=("PositionX PositionY PositionZ Radius "
                    "OrientX OrientY OrientZ "
                    "moi_volX moi_volY moi_volZ "
                    "startIndex endIndex") if first_clump else "",
            delimiter=" ")

        # -------------------------------------------------
        # 4. 写 pebble 文件
        # -------------------------------------------------
        first_pebble = not os.path.isfile(pebble_file_path)
        np.savetxt(
            pebble_file_path, pebble_data,
            header=("PositionX PositionY PositionZ Radius multisphereIndex")
            if first_pebble else "",
            delimiter=" ")
    # ========================================================= #
    #                        LEVELSET                           #
    # ========================================================= #
